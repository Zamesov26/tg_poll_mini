...
from app.dataaccess.questionnaires.service import QuestionnaireService
from app.telegram_bot.quizes.communication import send_random_poll, assign_lesson_questions, \
...
async def newsletter(message: Message):
    users = await UserRepository.find_all(state='registration')
    for user in users:
        print(user)
...
async def food_size_chosen(message: Message, state: FSMContext):
    await QuestionnaireService.add_one(message.from_user.id,
                                         'Прошлый опыт',
                                       message.text)
...
async def quiz_first(poll: PollAnswer, state: FSMContext):
    ...
    if not await send_random_poll(poll.bot, poll.user.id):
        # await assign_lesson_questions(poll.user.id_, 'roblox_exam_2')
        # await send_random_poll(poll.bot, poll.user.id_)
        # await state.set_state(Quiz.second)
        await state.clear()
        await UserRepository.update(poll.user.id, state='moved_to_second_stage',
                                    data={})
        await poll.bot.send_message(
            poll.user.id,
